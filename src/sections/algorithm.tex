...
\[
    t(n, d, 2) = \left\lfloor
        \left(  \frac{\log (n/2)}{\log (4/d)} \right)
       \right\rfloor.
\]
...
so $S, T \subset V$ are disjoint, of size $t$ and satisfy $S \times T \subset E$.
\begin{algorithm}[H]
    ...
    \label{alg:bipartite}
        ...
        \end{algorithmic}
\end{algorithm}
The minimum density $d \geq 3n^{-1/2}$ in~\ref{line:bipartite_min_d} is required
...
$P' = (X_1, X_2, \ldots, X_{k-1}) \in \mathcal{P}(V \setminus W)^{k-1}$ such that:
\begin{itemize}
    \item $|X_i| = t'$
    \item $X_1 \times \dots \times X_{k-1} \subset E(G') = S =
    \left\{x \in \binom{V \setminus W}{k-1} : \{x\} \cup y \in E \text{ for all } y \in T\right\}$
\end{itemize}
If we now concatenate $P'$ with $T$ (choosing a subset of $X_i$ of size $t$ for each $i$ if necessary), we get
a $k$-tuple of disjoint sets
\[P = X \circ (T) = (X_1, \ldots, X_{k-1}, T),\]
which satisfies $X_1 \times \dots \times X_{k-1} \times T \subset E = E(G)$
...
\begin{theorem}
    ...
    \begin{proof}
        ...
        reaches line~\ref{line:return} at some point.
