...
\Cref{fig:link} illustrates how to construct
...
\begin{remark}
    Condition~\eqref{eq:homomorphism} of~\Cref{def:embedding}
...
For more details, see~\Cref{apx:embeddings-properties}.
...
For example, in~\Cref{fig:complete_kgraph} we show the complete $3$-graph $\completesuperindex{3}{4}$.
...
\Cref{fig:222} shows the complete $3$-partite $3$-graph
with $2$ vertices in each part, $K(2, 2, 2) = K^{(3)}(2, 2, 2)$.
In contrast, ~\Cref{fig:k2_222} shows the complete $3$-partite $2$-graph
$K^{(2)}(2, 2, 2)$.
...
For example,~\Cref{fig:k2_222} displays the $2$-blowup of the triangle graph \completesuperindex{2}{3},
...
In particular, the complete $k$-graph $\compoverset{k}{t}$ (see~\Cref{fig:222} for an example)
