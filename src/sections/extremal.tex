...
The Turán number is increasing both in $n$ and under $k$-graph inclusion. % TODO
...
\begin{lemma} \label{lem:turan_complete_partite}
    ...
    \begin{proof}
        ...
        By~\Cref{lem:same_degree}, $d_G(x) = d_G(z)$ and $d_G(y) = d_G(z)$,
...
\begin{delayedproof}{thm:turan}
    We have shown in~\Cref{lem:turan_complete_partite} that $G = (V_1, \dots V_p; E)$ is complete.
...
\begin{remark}\label{rem:zar_vs_turan}
    ...
    Therefore, the expected number of edges satisfying this condition is a positive fraction of the number of edges $m$.
    ...
    \]
    Similar arguments with more careful approximations prove that in fact
    \[
        \ex{n}{\compdots{t_1}{t_k}} \leq \left( \frac{k^k}{k!} + o(1) \right) \cdot \zaroversetdots{k}{\lceil n / k \rceil}{t_1}{t_k}.
    \]

\end{remark}

The problem of finding the Zarankiewicz number was first posed by K. Zarankiewicz % TODO citation
in 1951 for the
...
to arbitrary complete partite $k$-graphs in the following theorem.
...
\begin{theorem} \label{thm:erdos64-constant-density}
    ...
    \begin{proof}
        ...
        We pick, for example,
        \[
             \gamma_k = \frac{2c_k}{e_k} = 2 \cdot 8^k \cdot k^2,
        \]
        so that
        \[
...
\begin{theorem} \label{thm:quant-blowup}
    ...
    \begin{proof}
        ...
        \]
        By~\Cref{thm:erdos64-constant-density}, making $n_0$ large enough depending only on $\delta_2$ and $|V|$,
...
\subsection{Open Problems}\label{subsec:open-problems} % TODO review
