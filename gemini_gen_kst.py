...
# --- Graph Definition ---
# LAYOUT CHANGE: Shifted all H coordinates left by 1.5 and up by 1.0
v_coords = {
    'A1': (-4.5, 6), 'A2': (-1.5, 8), 'A3': (1.5, 10),
    'B1': (-4.5, 1), 'B2': (-1.5, -1), 'B3': (1.5, -3),
...
# LAYOUT CHANGE: Increased vertical spacing to make H' taller
u_coords_hp = {f"{V1_nodes[i]}{V2_nodes[j]}": (15, 9.5 - (i * 3 + j) * 1.5) for i in range(3) for j in range(3)}
w_coords_hp = {V3_nodes[i]: (21, 7 - i * 3) for i in range(3)}
...
# --- TikZ Generation ---
lines = [
    r"% TikZ code for side-by-side dual proof sketch (v5 - Layout adjusted).",
...
# LAYOUT CHANGE: Moved H label to match new coordinates
...
lines.append(f"\\node[anchor=south] at (21, 8.5) {{{W_PARTITION_LABEL}}};")
for u in U_nodes:
    for w in W_nodes:
...
# LAYOUT CHANGE: Shifted the info box to match new H coordinates
...
# ... (Code for final slide is unchanged)
lines.append(f"\\only<{slide_counter}>{{")
...
try:
    ...
    with open(output_filename, "w") as f:
        f.write("\n".join(lines))
    print(f"Success: New animated TikZ code with layout adjustments written to '{output_filename}'")
