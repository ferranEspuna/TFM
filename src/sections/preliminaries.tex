...
\begin{definition}
    ...
    Let $S \subset V$ be a set of vertices and let $1 \leq j \leq k - 1$.
    The \emph{common $j$-link} of $S$ is the $j$-graph $G_S^j = (V \setminus S, E')$, where
    \[
        E' = \left\{Y \in \binom{V \setminus S}{j}\middle\vert \, X \cup Y \in E \text{ for all } X \in \binom{S}{k-j}\right\}.
    \]
\end{definition}

Then, we can define completeness of $k$-partite $k$-graphs recursively: Let $G = (V_1, \dots, V_k; E)$
be a $k$-partite $k$-graph.
If $k=1$, $G$ is complete if and only if $E = V_1$ (as in Definition~\ref{def:complete_kpartite}).
Otherwise, $G$ is complete if and only if $G_{V_k}^{k-1}$ (which is a $k-1$-partite $k-1$-graph
...
\begin{remark}\label{rem:zar_vs_turan}
    ...
    Assume that $H$ is a $K(t_1, \dots, t_k)$-free $n$-vertex $k$-graph with $m$ edges.
    ...
    for an edge $e \in E(H)$, the probability that $e$ is an edge in $K(V_1, \dots, V_k)$ is
    greater than
    ...
    \[ex(n, K(t_1, \overset{k}{\dots}, t_k)) = O(z(\lceil n / k \rceil , \overset{k}{\dots}, \lceil n / k \rceil; t_1, \dots, t_k)).\]
...
\begin{remark}
    ...
    \[
        ex(n, K(s, t)) =
        O\left((s - 1)^{1 / t}\left(\left\lceil\frac{n}{2}\right\rceil - t + 1\right)n^{1 - 1 / t} + (t - 1)\left\lceil\frac{n}{2}\right\rceil\right) =
        O\left(n^{2 - 1 / t}\right).
    \]
...
\begin{theorem}\label{thm:erdos64}
    ...
    $ex(n, K(t, \overset{k}{\dots}, t)) = O\left(n^{k - \frac{1}{t^{k-1}}}\right)$.
    \begin{proof}
        By Remark~\ref{rem:zar_vs_turan}, it suffices to show that
        \[
            z = z(w, \overset{k}{\dots}, w; t, \overset{k}{\dots}, t) = O\left(w^{k - \frac{1}{t^{k-1}}}\right).
        \]
