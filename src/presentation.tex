...
\begin{frame}
    ...
    \uncover<2->{
        We want to \textbf{find} $\compoverset{k}{t}$ in $H = (V, E)$ of constant density $d = m/n^k$.
    }

    \uncover<3->{
        Naïve approach: enumerate all $k$-partite $k$-subgraphs of $H$.
    }
...
\begin{frame}
    ...
    \begin{itemize}
        \item<2-> Find a bipartite subgraph $(U, W; E)$ of $H$ with many edges.
        \item<3-> For each subset $T \in \binom{W}{t}$, calculate the common neighborhood.
        For some $T$ it has size $s = t$.
    \end{itemize}

    \uncover<4->{\textbf{Problems}:}
    \begin{itemize}
        \item<5-> Finding a dense bipartite subgraph.
        \item<6-> If we are not careful, the search space $\binom{W}{t}$ might still be too large \dots
    \end{itemize}

    \uncover<7->{\textbf{Solution}: $W$ are the $\mathbf{w = \lfloor t / d \rfloor}$ vertices of \textbf{maximum degree}. $U = V \setminus W$.}
    \begin{itemize}
        \item<8-> Max degree $\implies$ The bipartite subgraph has many edges.
        \item<9-> $w$ is ``just right''.
        If too small, the extra term in the $KST$ bound is too large; if too big, the search space $\binom{W}{t}$ is too large.
    \end{itemize}
\end{frame}
...
\begin{frame}[plain] % The 'plain' option removes the headline and footline for a clean look
    ...
    \rule{\textwidth}{0.4pt} % A thin horizontal line
    \vspace{0.2cm}
    \normalsize
    Ferran Espuña Bertomeu \\
    \texttt{ferranespuna@gmail.com} % Replace with your actual email
\end{frame}
