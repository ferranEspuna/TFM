...
\begin{remark}\label{rem:zar_vs_turan}
    ...
    Applying the first moment method, there is at least one partition retaining $ \frac{k!}{(2k)^k} m$
    edges.
...
\begin{delayedproof}{thm:erdos64}
    ...
    \[
        ...
        \leq \frac{k \cdot 4^k}{(k-1)!} \cdot n^{k - \frac{1}{t^{k-1}}}.
    \]
...
A stronger version of Theorem~\ref{thm:erdos64}, which also follows from Lemma~\ref{lem:erdos64-quant},
is the following.
If $H_n$ is a family of $k$-partite $k$-graphs with constant positive density
...
\[
    t_n = \delta \cdot (\log n)^{\frac{1}{k-1}}
\]
for some constant $\delta = \delta(k, \epsilon) > 0$.

Loosely speaking, we may find a blow-up of an edge as large as we wish, if we let the number of vertices grow
...
\begin{theorem} \label{thm:quant-blowup}
    ...
    \begin{proof}
        ...
        Assuming that that $n \geq n_0 \geq 2|V|$,
        ...
        \]
        By Lemma~\ref{lem:erdos64-quant}, making $n_0$ large enough depending only on $\delta_2$ and $|V|$,
