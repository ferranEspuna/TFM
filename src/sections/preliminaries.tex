...
\begin{remark} \label{rem:uniform-t}
    ...
    gives
    \[
        z(w, w, t, t) \leq (t-1)^{1/t}(w-t+1)w^{1-1/t} + (t-1)w \leq w^{2 - 1/t} + o\left(w^{1-1/t}\right).
    \]
...
\begin{theorem}
    Let $G$ be a hypergraph and let $H_n$ be a family of graphs with $n$
    vertices and $\left( \pi(G) + \Omega (1) \right) \binom{n}{k}$ edges.
    Then, for $n$ large enough, $H_n$ contains $G(t_n)$ as a subgraph,
    where $t_n = \Omega\left( (\log n)^{\frac{1}{k-1}} \right)$.
    \begin{proof}
        TODO. Use supersaturation + probabilistic method + remark. % TODO
    \end{proof}
\end{theorem}
...
In particular, this directly proves the Erdős-Stone-Simonovits theorem for $2$-graphs (Theorem~\ref{thm:erdos_stone_simonovits}).
Indeed, any graph with chromatic number $r$ is a subgraph of
