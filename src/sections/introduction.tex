...
Given a fixed $k$-graph $G$, the \emph{Turán number} $\ex{n}{G}$
denotes the maximum number of edges in a $k$-graph on $n$ vertices that does not contain $G$ as a subgraph.
...
provides an upper bound $\ex{n}{K(s, t)} = O(n^{2 - 1/\min\{s,t\}})$ for $k=2$.
Erdős~\cite{Erods1964} generalized this type of bound to higher uniformities,
showing that $\ex{n}{\compoverset{k}{t}} = O(n^{k - 1/t^{(k-1)}})$ for $k \ge 2$.
...
indicating that $k$-partite graphs have Turán density zero,
there is often a significant gap between these bounds and the best-known lower bounds.
Lower bounds, frequently derived from probabilistic methods or algebraic constructions
~\cite{kollar1996norm, brown1966graphs, conlon2020random},
...
Furthermore, the proofs of the upper bounds, like the one presented for~\Cref{thm:erdos64}
...
The size $t$ of the parts found by the algorithm depends on the uniformity $k$, the number of vertices $n$,
and the edge density of the host graph, given precisely by Equation~\eqref{eq:t}.
...
\Cref{sec:preliminaries} formally introduces some families of hypergraphs, including complete $k$-partite hypergraphs, as well as some basic notation.
