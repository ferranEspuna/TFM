...
\begin{theorem} % TODO more detail?
    ...
    \begin{proof}
        ...
        first note that for $k = 1$, the algorithm clearly runs in linear time.
        For $k > 1$, we first argue that the for loop in~\cref{line:for} runs in polynomial time.
        ...
        \[
            ...
            e^{4 \log n} = n^4.
        \]
        Let us assume that $H$ is given as an adjacency matrix.
        Calculating the degree of each vertex in $H$ takes $\bigO{n^{k-1}}$
        steps and calculating the total number of edges $m$ takes $\bigO{n^{k}}$ steps.
        Calculating the parameters $d, t, w, s$ in~\cref{line:tws} thus takes $\bigO{n^{k}}$ steps.
        Finding a set $W$ of $w$ vertices with highest degree in $H$ takes, using a priority queue,
        $\bigO{n \log w}$ steps, assuming the degree of each vertex has been already calculated
        and stored in memory.
        Given $t$ vertices in $W$, we can obtain the set of edges containing each one in $\bigO{n^{k-1}}$ steps,
        and intersect them in $\bigO{tn^{2(k-1)}}$ steps, running indices on the common intersection
        and each set to remove edges iteratively.
        Then, constructing the adjacency matrix of the new graph $H'$ in~\cref{line:rec_call} from this edge list
        takes $\bigO{n^{k-1}}$ steps.
        This means that if $R_k(n)$ is the running time of the algorithm for $k$-graphs with $n$ vertices,
        we have the recurrence
        \[
            R_k(n) = \bigO{n^k + n \log w + n^4 (tn^{2(k-1)} + n^{k-1}) + R_{k-1}(n - w)}
            = \bigO{n^{2k+3} + R_{k-1}(n - w)},
        \]
        Because the algorithm performs only one recursive call.
        It is now straightforward to see that
        \[
            R_k(n) = \bigO{n^{2k+3}}. \qedhere
        \]
    \end{proof}
\end{theorem}
