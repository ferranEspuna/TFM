...
\begin{equation} \label{eq:d}
    d = m/n^k
\end{equation} is the ``un-normalized'' density of $G$,
...
\begin{theorem}\label{thm:kpartite}
    ...
    That is, the algorithm returns a tuple of sets $(V_1, \ldots, V_k) \subset \binom{V}{t}^k$ such that
    $V_1 \times \cdots \times V_k \subset E$.
...
\[
    ex(n, K(t, \overset{k}{\dots}, t)) < m =  O\left(n^{k - \frac{1}{t^{k-1}}}\right).
\]
...
there may not even be a $K(2, 2)$ in $G$. % TODO: Explain this better in preliminaries
...
yields a $K(t', \overset{k-1}{\dots}, t')$ in $G'$ with $t' \geq t$.
That is, a tuple
...
a $k$-tuple $(X_1, \ldots, X_{k-1}, T)$, of $t-sets$ of $V$ % todo k-tuple is split
which by the definition of $S$ satisfies
$X_1 \times \dots \times X_{k-1} \times T \subset E = E(G)$
so it forms a $K(t, \overset{k}{\dots}, t)$ in $G$.
        ...
        \Function{FIND\_PARTITE}{$G, k$}
            ...
            \ForAll{$T \in \binom{W}{t}$} \label{line:for}
                ...
                \If{$|S| \geq s$}
                    ...
                    \State $(V_1, \ldots, V_{k-1}) \gets$ \Call{FIND\_PARTITE}{$G', k-1$}
...
This means that, assuming that the algorithm finds a $K(t', \overset{k-1}{\dots}, t')$ in $G'$ in the recursive call,
it finds a $K(t, \overset{k}{\dots}, t)$ in $G$.
...
\begin{theorem}
    ...
    \begin{proof}
        ...
        $(X_1, X_2, \ldots, X_{k-1}) \in \mathcal{P}(V)^{k-1}$ such that $|X_i| \geq t(n-w, d', k-1) \geq t$
        for all $i$ and $X_1 \times \dots \times X_{k-1} \subset E(G')$.
        However, by construction of $G'$,
         \[
             E(G') = S = \left\{x \in \binom{V \setminus W}{k-1} : \{x\} \cup y \in E \text{ for all } y \in T\right\}.
         \]

        That is, the tuple $(X_1, \dots, X_{k-1}, T)$ returned in line~\ref{line:return}
