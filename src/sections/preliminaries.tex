...
\begin{definition}
    ...
    The \emph{common neighborhood} of a set $T \subset V$ is
    \[
        N_G(T) = \{y \in V \mid \forall x \in T: \{x, y\} \in E\}.
    \]
\end{definition}

That is, the common neighborhood of a set $T \subset V$ is the maximal set $S \subset V \setminus T$ such that
$T \times S \subset E$.
...
\begin{definition}
    ...
    Let $T \subset V$ be a set of vertices and let $1 \leq j \leq k - 1$.
...
\begin{figure}[htbp]
    ...
    \caption{A $3$-graph $G$ and the common $2$-link graph $\link{G}{T}{2}$ of the set $T = \{A, B\}$.
        Vertices are represented as black dots, and $3$-edges of $G$ are represented as colored or gray small dots,
        connected by a line to the vertices they contain. Colored dots correspond to $3$-edges with exactly
        ...
        in the same color that the $3$-edges they come from. Dashed lines correspond to edge pairs
        in $V \setminus T$ that have some of the required $3$-edges in $G$, but not all of them.
        In conclusion, the link graph has vertex set $\{X, Y, Z, W\}$ and edge set
        $\{\{X, Y\}, \{Z, W\}\}$.
