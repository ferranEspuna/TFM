...
\begin{definition}\label{def:zarankiewicz}
    let $0 < t_1 \leq n_1, \dots, 0 < t_k \leq n_k$ be integers.
    Then the \emph{generalized Zarankiewicz number} $z(n_1, \dots, n_k; t_1, \dots, t_k)$
    ...
    such that no embedding $f$ of $K(W_1, \dots, W_k)$ with $|W_i| = t_i$ in it exists
...
\begin{remark}\label{rem:zar_vs_turan}
    ...
    pick $n_1, \dots, n_k$ such that $\sum_{i} n_i = n $ and $n_i \sim n/k $.
    ...
    Applying the probabilistic method, we can conclude that
    \[ex(n, K(t_1, \dots, t_k)) = O(z(n_1, \dots, n_k; t_1, \dots, t_k))\]

\end{remark}


% TODO CHANGE VARIABLES m, n; s, t => u, v; p, q
% This will make notation more consistent with the rest of the thesis
...
\begin{theorem}\label{thm:kst}
    ...
    \begin{proof}
        Suppose that we have a bipartite graph $G = (M, N; E)$
        with $|M| = m$, $|N| = n$ and $|E| = z$ exceeding the bound stated above.
        Let us consider the set
        \[
            P = \left\{ (x, Y) \in M \times \binom{N}{t}
            \Big| \forall y \in Y: \{x, y\} \in E \right\}
        \]
        Counting on the first coordinate, and using Jensen's inequality, we get
        \[
            |P|
            = \nsum_{x \in M} \binom{d_G(x)}{t}
            = \nsum_{x \in M} f(d_G(x))
            \geq m  \nsum_{x \in M} f(z/m)
            = m \binom{z / m}{t}
        \]
        ...
        This would mean that the inclusion map from $ S \cup T$ to $M \cup N$ is an embedding of
        ...
        \]
        Now, because we can see $E$ as a subset of $M \times N$,
