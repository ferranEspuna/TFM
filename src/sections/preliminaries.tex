...
\begin{lemma}
    Let $G$ be a $K_r^{(2)}$-free $2$-graph with $\ex{n}{\completesuperindex{2}{r}}$ edges.
    $G$ is $p$-partite for some $p \geq 2$.
    \begin{proof}
        Equivalently, we show that the relation defined by non-adjacency on $V(G)$ (that is, $x \sim y$ when
        $\{x, y\} \notin E(G)$) is an equivalence relation, so we can divide $V(G)$ into equivalence classes
        ...
        Suppose, by way of contradiction, that there exist $x, y, z \in V(G)$ such that
        ...
        Suppose it does.
        Because $z$, $z_1$ and $z_2$ are all disconnected, only one of them can be
        ...
        Without loss of generality, $G'[\{z_1, x_2, x_3, \dots, x_r\}]$ is complete.
        However, $\{z_1, x_i\}$ is an edge in $G'$ if an only if $\{z, x_i\}$ is an edge in $G$,
        which means that $G[\{z, x_2, x_3 \dots, x_r\}]$ is complete, against our assumption.

        Finally, we show that $G'$ has more edges than $G$.
        TODO %TODO
    \end{proof}
\end{lemma}
...
\begin{proof}[Proof of Turán Theorem]
     We first note that $\compdotssuperindex{2}{a_1}{a_{r-1}}$ is a $K_r^{(2)}$-free $2$-graph.
     This is because the first is $(r-1)$-colorable, while the second is not. % TODO colorable skips the line
     TODO % TODO
\end{proof}
