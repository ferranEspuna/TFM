...
% TODO discussion of constant density case. Is it optimal?

For $k = 2$, this algorithmic problem was already solved by Mubayi and Turán~\cite{MUBAYI2010174}.
Their algorithm follows very closely the structure of the proof of Theorem~\ref{thm:kst}.
It partitions $V(G)$ into two sets $U$ and $W$ such that there are many edges between them
and the size of $W$ is logarithmic in $n$.
This is achieved by selecting a set of vertices of highest degree, that
is, no vertex in $U$ has more neighbors than any vertex in $W$.
Then, by iterating over all $t$-subsets of $W$ it finds such a set $T$ and an appropriate $t$-set $S \subset U$,
for which $S \times T \subset E$.
We present Mubayi and Turán's algorithm (Algorithm~\ref{alg:bipartite}) here for context and clarity.
...
We require that $d \geq 3n^{-1/2}$ in~\ref{line:bipartite_min_d} because if $d = o\left(n^{-1/2}\right)$ then
...
To see that the algorithm returns a pair of sets $(S, T)$, one first shows that
...
This in turn means that for some iteration of the loop, the size of $S$ is at least $t$
...
\end{equation}
Then, for every $t$-subset $T$ of $W$, we find the set $S$ of $(k-1)$-subsets of $V \setminus W$
that form an edge with every vertex in $T$.
For a specific $T$, it will be true that
\begin{equation}
    \label{eq:s}
    |S| \geq s = s(n, d, k) = \left\lfloor d^{t(n, d, k)} n^{k-1} \right\rfloor
\end{equation}
This will allow us to define a new $(k-1)$-graph $G'$ with vertex set $V \setminus W$ and edge set $S$.
As it turns out, applying the algorithm recursively to $G'$
will yield a $K(t, \overset{k-1}{\cdots}, t)$ in $G'$.
This is, a tuple of pairwise disjoint sets
...
That is, the returned tuple $(X_1, \dots, X_{k-1}, T)$
satisfies $X_1 \times \dots \times X_{k-1} \times T \subset E = E(G)$
...
% TODO 1-graph is a subset of a set
% TODO explain what \circ means
