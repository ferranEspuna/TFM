...
\begin{remark}
    These chromatic numbers are increasing under the inclusion $k-graphs$.
...
There are even fewer solved cases than in the non-degenerate case.
Among them, it is known that, for $K(2, t)$ (for $t \geq 2$) and $K(3, 3)$, Theorem~\ref{thm:kst} is optimal in the sense that
\begin{equation} \label{eq:k22-optimal}
    \ex{n}{K(2, t)} = \Theta\left(n^{\frac{3}{2}}\right)~\cite{erdHos1966problem, furedi1996new},
\end{equation}
and also
\begin{equation} \label{eq:k33-optimal}
...
The main difficulty for degenerate problems is that lower bounds for the Turán numbers
often rely on specific geometric or algebraic constructions such as the ones
cited for $K(2, 2)$ and $K(3, 3)$.
General lower bounds are usually very weak and do not reach the correct order of magnitude.
For example, see the following probabilistic construction.

\begin{proposition}
    $\zardots{w}{w}{t_1}{t_k} = \Omega \left(w^{k - \frac{\sum_i t_i}{\prod_i t_i}}\right)$ as $w \to \infty$.
    \begin{proof}
        Let $W_i$ be disjoint sets with $|W_i| = w \geq t_i$ for all $i$
        and consider the following probability distribution on the set of $k$-partite $k$-graphs
        with parts $W_1, \dots, W_k$.
        Any edge $e \in W_1 \times \dots \times W_k$
        is included in the edge set with probability
        \[
            p = \frac{1}{3}w^{-\frac{\sum_i t_i}{\prod_i t_i}}
        \]
        independently of the others.
        The expected number of copies of $\compdots{t_1}{t_k}$ in the resulting graph is
        \[
           p^{\prod_i t_i} \prod_i \binom{w}{t_i}
           \leq  p^{\prod_i t_i} \prod_i w^{t_i}
           = p^{\prod_i t_i} w^{\sum_i t_i} \leq \frac{1}{3},
        \]
        which means that the resulting graph is $\compdots{t_1}{t_k}$-free with probability at least $2/3$.
        On the other hand, the number of edges of the graph follows a binomial distribution with mean
        \[
            pw^k = \frac{1}{3}w^{k - \frac{\sum_i t_i}{\prod_i t_i}}.
        \]
        Therefore, the event that the graph has more than this number of edges happens with probability
        at least $1/2$.
        This means that the intersection of the two events has positive probability, so there
        exists a $k$-partite $\compdots{t_1}{t_k}$-free $k$-graph with part sizes $w$ and more than $pw^k$ edges,
        as desired.
...
\[
    \ex{n}{\compdots{t_1}{t_k}} = \Omega\left(n^{k - \frac{\sum_i t_i}{\prod_i t_i}}\right).
\]
