...
\begin{theorem}[Turán Theorem]
    ...
    \begin{proof}
        ...
        Next, we claim that if $G$ is a $K_r^{(2)}$-free $2$-graph which is maximal with respect to inclusion
        (which is implied by it having the maximum number of edges),
        then $G$ is $(r-1)$-partite.
    \end{proof}
\end{theorem}
