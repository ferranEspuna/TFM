...
In essence, if a $k$-graph $H$ has $\left(\pi(G) + \Omega(1) \right) \binom{n}{k}$ edges,
...
\begin{theorem}[Supersaturation] \label{thm:supersaturation}
    ...
    There exists a positive integer $n_0(G, \epsilon)$ and a constant $\delta(G, \epsilon) > 0$
    such that any graph $H_n$ with $n \geq n_0$ vertices and at least $\left(\pi(G) + \epsilon \right) \binom{n}{k}$ edges
    contains at least $\delta n^{|V|}$ copies of $G$ as a subgraph.

    \begin{proof}
        Pick $t$ large enough so that $\ex{t}{G} \leq \left(\pi(G) + \frac{\epsilon}{2}\right) \binom{t}{k}$.
        Suppose, furthermore, that $n \geq t$.
        Let m be the number of edges of $H_n$.
        Notice that
        \[
        ...
        We define
        \[
            P = \left\{ T \in \binom{V}{t} \middle| E(H_n[T]) > \left(\pi(G) + \frac{\epsilon}{2}\right) \binom{t}{k} \right\}.
        \]
...
(that is, $E(H_n) \geq \epsilon \binom{n}{k} = \Omega\left( n^k \right)$ for some $\epsilon > 0$),
for large enough $n$, $H_n$ contains a $\compoverset{k}{t_n}$ as a subgraph, with
\[
    t_n = \delta(\epsilon, k) \cdot (\log n)^{\frac{1}{k-1}}
\]
for some constant $\delta(\epsilon, k) > 0$.
The following theorem generalizes this notion to blow-ups of arbitrary $k$-partite $k$-graphs.

\begin{theorem} \label{thm:quant-blowup} % TODO epsilon delta
    Let $G = (V, E)$ be a $k$-graph and let $H_n$ be a family of $k$-graphs with $n$
    vertices and $\left( \pi(G) + \Omega (1) \right) \binom{n}{k}$ edges. % Again, Richard says this is not correct. Why?
