...
\abstracteng{
    ...
    asking how many edges a $k$-graph $H$ can have, as a function of its number of vertices $n$,
    without containing a specific subgraph $G$.
    All graphs have a constant $\pi(G)$ called the \emph{Turán density},
    such that this number is $(\pi(G) + o(1)) \binom{n}{k}$ as $n \to \infty$.
    This thesis deals with degenerate Turán problems ($\pi(G)=0$)
    by focusing on finding complete balanced $k$-partite $k$-subgraphs,
    denoted $\compoverset{k}{t}$, within a $k$-uniform hypergraph ($k$-graph).
