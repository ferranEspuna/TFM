...
\abstracteng{
    ...
    They ask for the maximum number of edges a $k$-graph $H$ can have,
    as a function of its number of vertices $n$,
    without containing a specific subgraph $G$, denoted $\ex{n}{G}$.
    ...
    explicitly depends on $n$, $k$, and the density $d=m/n^k$, matching the $(\log n / \log(1/d))^{1/(k-1)}$
    scaling from existence proofs.
    Our method generalizes work on the bipartite case by Mubayi and Turán, using a recursive strategy on link graphs.
}
