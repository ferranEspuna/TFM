...
\begin{lemma}\label{lem:same_degree}
    ...
    \begin{proof}
        ...
        suppose that $G'[T']$ contains is complete for some $T' \subset V'$ of size $r$.
        ...
        we can assume that $T$ contains $x'$ and not $x$.
        Now, let $T = (T' \setminus \{x'\}) \cup \{x\} \subset V$, also of size $r$.
        Then, the graph $G[T] = G'[T]$ must be complete, reaching a contradiction.
    \end{proof}
\end{lemma}
...
The following general theorem states that a similar result holds for all $k$-graphs.

\begin{theorem}
    Let $G = (V, E)$ be a $k$-graph.
    The limit
    \[
        \alpha(G) = \lim_{n \to \infty} \frac{\ex{n}{G}}{\binom{n}{k}}
    \]
...
% TODO Erdős-Stone theorem, Erdos-Stone-Simonovits theorem, conjectures for uniformity 3
