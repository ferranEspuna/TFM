...
\begin{theorem} \label{thm:erdos64-constant-density}
    ...
    \begin{proof}
        ...
        \[
            \epsilon \binom{n}{k} \geq \epsilon \frac{(n - k + 1)^k}{k!} \geq \epsilon \left( \frac{k-1}{k} \right)^k \frac{1}{k!} n^{k} = (e_k  \cdot \epsilon) n^{k},
        \]
        where $e_k = \frac{(k-1)^k}{k!}$ does not depend on $n$.
        We pick
        \[
             \gamma_k = \frac{c_k}{e_k},
        \]
        so that
        \[
            \ex{n}{\compoverset{k}{t_n}} \leq c_k n^{k - \frac{1}{t_n^{k-1}}} \leq c_k n^k  \frac{\epsilon}{\gamma_k} = e_k \cdot \epsilon n^{k} \leq |E(H)|,
        \]
