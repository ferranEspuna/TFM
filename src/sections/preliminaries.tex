...
\begin{theorem}[Supersaturation \cite{erdHos1983supersaturated}] \label{thm:supersaturation}
    ...
    \begin{proof}
        ...
        For $t$ large enough, $\ex{t}{G} \leq (\pi(G) + \gamma) \binom{t}{k}$.
        Suppose that $n \geq t$.
        Notice that
        \[
        ...
        We define
        \[
            P = \left\{ T \in \binom{V}{t} \middle| E(H_n[T]) > (\pi(G) + \gamma) \binom{t}{k} \right\}.
        \]
        If $T \in \binom{V}{t}$, the number of edges in $H_n[T]$ is at most $\binom{t}{k}$.
        Therefore,
        \[
            \binom{n-k}{t-k}(\pi(G) + \gamma)\binom{n}{k} \leq \binom{n-k}{t-k}m \leq |P| \left( (\pi(G) +  \gamma)\binom{t}{k}\right) + (n - |P|) \binom{t}{k}.
        \]
    \end{proof}
\end{theorem}
