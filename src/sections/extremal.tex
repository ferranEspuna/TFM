...
The Turán number is increasing both in $n$ and under $k$-graph inclusion. % TODO Gemini says this is wrong
...
\begin{remark}
    Following~\Cref{rem:zar_vs_turan}, we can use this bound to get an upper bound on the Turán number of $K(s, t)$:
    \[
        \ex{n}{K(s, t)} =
...
(pictured in~\Cref{fig:complete_kgraph}) or the unique $3$-graph $\completesuperindex{3}{4-}$
...
\Cref{thm:kst} is optimal in the sense that
\[
    \ex{n}{K(2, t)}
...
\]
and, by~\Cref{prop:probabilistic-lower-bound}, we get
